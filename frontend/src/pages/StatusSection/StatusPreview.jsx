...
const StatusPreview = ({
  ...
  const curerntStatus = contact?.statuses[currentIndex];
  ...
  const handleDeleteStatus = () => {
    if (onDelete && curerntStatus?.id) {
      onDelete(curerntStatus.id);
    }
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className={`fixed inset-0 w-full h-full bg-black bg-opacity-90 z-50 flex items-center
         justify-center`}
      ...
      >
        ...
        >
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-300 text-sm">
                  {formatTimestamp(curerntStatus.timeStamps)}
                </p>
              </div>
            </div>
            {isOwner && (
          ...
          <div className="w-full h-full flex items-center justify-center">
            {curerntStatus.contentType === "text" ? (
              <div className="text-white text-center p-8 ">
                <p className="text-2xl font-medium">{curerntStatus.media}</p>
              </div>
            ) : curerntStatus.contentType === "image" ? (
              <img
                src={curerntStatus.media}
                alt="image-video"
                className="max-w-full max-h-full object-contain"
              />
            ) : curerntStatus.contentType === "video" ? (
              <video
                src={curerntStatus.media}
                alt="image-video"
                controls
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 text-white bg-black bg-opaicty-50 rounded-full p-3 hover:bg-opacity-70 transition-all z-10"
          >
          ...
          {currentIndex > 0 && (
            <button
              onClick={onPrev}
              className="absolute left-4 top-1/2 transition -translate-y-1/2 text-white bg-black bg-opaicty-50 rounded-full p-3 hover:bg-opacity-70"
            >
          ...
          {currentIndex < contact.statuses.length - 1 && (
            <button
              onClick={onNext}
              className="absolute right-4 top-1/2 transition -translate-y-1/2 text-white bg-black bg-opaicty-50 rounded-full p-3 hover:bg-opacity-70"
            >
            ...
            <div className="absolute bottom-4 left-4 right-4">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span>{curerntStatus?.viewers.length}</span>
                </div>
                <FaChevronDown
              ...
              <AnimatePresence>
                {showViewers && (
                  ...
                  >
                    ...
                    ) : curerntStatus.viewers.length > 0 ? (
                      <div className="space-y-2">
                        {curerntStatus?.viewers.map((viewer) => {
                    return <div
                            key={viewer._id}
